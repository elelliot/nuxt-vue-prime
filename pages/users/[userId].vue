...
// const { data: allUsers, pending: pendingAll } = await useFetch(
//   `/api/users/all`
// );
...
async function confirmUpdate(name, email, password) {
  const resp = await $fetch(`/api/users/${route.params.userId}`, {
    method: "PUT",
    body: {
      name,
...
const showUpdateConfirmation = (name, email, password) => {
  confirm.require({
    ...
    accept: () => confirmUpdate(name, email, password),
  });
};
...
const showDeleteConfirmation = () => {
  confirm.require({
    message:
      "You're about to delete your account, are you really sure?, this action cannot be undone",
